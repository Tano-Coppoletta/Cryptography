...
from mysecrets import cbc_oracle_ciphertext2 as ciphertext
...
def guess_byte(p,c,ciphertext,block_size):
    ...
    for i in range(0,256):
        ...
        for x in p:
            if j == len(p):
                ca += (x ^ last_padding_value).to_bytes(1,byteorder='big')
            else:
                ca += (x ^ 0).to_bytes(1, byteorder='big')
            j+=1
        # print(ca)
