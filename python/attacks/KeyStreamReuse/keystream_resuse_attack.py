...
#############################################àà
#guess the first byte: naive approach: count the ascii char
counters = numpy.zeros(256, dtype=int)

for guessed_byte in range(256):
    for c in ciphertexts:
        if chr(c[0] ^ guessed_byte) in ascii_uppercase:
...
candidates = []
#save all the candidates (most frequent chars)
for pair in ordered_match_list:
    if pair[0] < max_matches * 0.95:
        break
    ...
    for guessed_byte in range(256):
        for c in ciphertexts:
            if chr(c[byte_to_guess] ^ guessed_byte) in printable:
                counters[guessed_byte] += 1
                #we are assuming that all the letters have the same weight
...
#try to build the key_steam
key_stream = b''
for x in candidates_list:
    key_stream += x[0][1].to_bytes(1,byteorder='big')

from Crypto.Util.strxor import strxor

for c in ciphertexts:
    print(strxor(c[:min_len],key_stream))
